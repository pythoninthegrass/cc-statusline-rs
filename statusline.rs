...
fn statusline(short_mode: bool, show_pr_status: bool) -> String {
    ...
    let current_dir = input.get("workspace")
    ...
    let model_display = if let Some(model) = model {
        ...
        let pct_color = if pct_num >= 90.0 { "\x1b[31m" }
        else if pct_num >= 70.0 { "\x1b[38;5;208m" }
    ...
    let current_dir = match current_dir {
        Some(dir) => dir,
        None => return format!("\x1b[36m~\x1b[0m{}", model_display),
    };
    
    // Check if git repo
    if !is_git_repo(current_dir) {
        let display_path = current_dir.replace(&home_dir(), "~");
        return format!("\x1b[36m{}\x1b[0m{}", display_path, model_display);
    }
    ...
    let session_summary = if let (Some(session_id), Some(transcript_path)) = (session_id, transcript_path) {
        if !git_dir.is_empty() {
            get_session_summary(transcript_path, session_id, &git_dir, current_dir)
                .map(|summary| format!(" \x1b[90m• \x1b[38;5;75m{}\x1b[0m", summary))
                .unwrap_or_default()
        } else {
            String::new()
        }
    ...
    let session_id_display = if let Some(session_id) = session_id {
        format!(" \x1b[90m• {}\x1b[0m", session_id)
    } else {
        String::new()
    };
    ...
    let pr_display = if pr_url.is_empty() {
        ...
        format!(" \x1b[90m• {}\x1b[0m", pr_url)
    };
    
    let pr_status_display = if pr_status.is_empty() {
        String::new()
    } else {
        format!(" {}", pr_status)
    };
    ...
    for line in &lines[start..] {
        ...
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(line) {
            if let (Some(ts), Some(usage), Some(role)) = (
                ...
                json.get("message").and_then(|m| m.get("role")).and_then(|r| r.as_str())
            ) {
                if role == "assistant" {
...
fn get_pr(branch: &str, working_dir: &str) -> String {
    ...
    if git_dir.is_empty() { return String::new(); }
    ...
    if let Ok(ts_content) = fs::read_to_string(&ts_file) {
        if let Ok(cached_ts) = ts_content.trim().parse::<u64>() {
            ...
            if now - cached_ts < 60 {
                return fs::read_to_string(&cache_file).unwrap_or_default().trim().to_string();
            }
        }
    }
...
fn get_git_status(working_dir: &str) -> String {
    ...
    if status_output.is_empty() { return String::new(); }
    ...
    if added > 0 { result.push_str(&format!(" +{}", added)); }
    if modified > 0 { result.push_str(&format!(" ~{}", modified)); }
    if deleted > 0 { result.push_str(&format!(" -{}", deleted)); }
    if untracked > 0 { result.push_str(&format!(" ?{}", untracked)); }
...
fn get_session_duration(transcript_path: Option<&str>) -> Option<String> {
    ...
    if !Path::new(transcript_path).exists() { return None; }
                ...
                if role == "user" {
                    let content_text = if let Some(text) = content.as_str() {
                        ...
                        if let Some(first) = array.first() {
                            first.get("text").and_then(|t| t.as_str()).unwrap_or("").trim().to_string()
                        } else {
                            continue;
                        }
                    } else {
...
fn get_session_summary(transcript_path: &str, session_id: &str, git_dir: &str, working_dir: &str) -> Option<String> {
            ...
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&checks_json) {
                if let Some(array) = parsed.as_array() {
                    ...
                    if let Some(fail) = groups.get("fail") {
                        ...
                        let count = if fail.len() > 1 { fail.len().to_string() } else { String::new() };
                        status.push_str(&format!("\\x1b[31m✗{}:{}{}\\x1b[0m ", count, names, more));
                    }
                    
                    if let Some(pending) = groups.get("pending") {
                        let names = pending.iter().take(3).cloned().collect::<Vec<_>>().join(",");
                        let more = if pending.len() > 3 { "..." } else { "" };
                        let count = if pending.len() > 1 { pending.len().to_string() } else { String::new() };
